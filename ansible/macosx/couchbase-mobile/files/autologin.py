...
# Enable mac autologin via kcpassword and com.apple.loginwindow.plist
# kcpassword generator is based on Tom Taylor's
...
def autologin(userid):
    cmd = "/usr/bin/defaults write /Library/Preferences/com.apple.loginwindow autoLoginUser " + userid
    os.system(cmd)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--username', required=True, help='user id')
    parser.add_argument('--password', required=True, help='user password')
    args = parser.parse_args()

    passwd = kcpassword(args.password)
    fd = os.open('/etc/kcpassword', os.O_WRONLY | os.O_CREAT, 0o600)
    file = os.fdopen(fd, 'w')
    file.write(passwd)
    file.close()

    autologin(args.username)
